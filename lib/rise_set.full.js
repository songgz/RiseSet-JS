...
// apparent position of the Sun in geocentric coordinates [degrees]
riseset.solarPosition = function(ajd) {
    var t = (ajd-2451545.0)/36525;
    var l0 = (280.46645 + 36000.76983*t + 0.0003032*t*t) % 360;
    if (l0<0) {
        l0 += 360;
    }
    var m = (357.52910 + 35999.05030*t - 0.0001559*t*t - 0.00000048*t*t*t) % 360;
    if (m<0) {
        m += 360;
    }
    var mr = riseset.d2r(m);
    var e = 0.016708617 - 0.000042037*t - 0.0000001236*t*t;
    var c = (1.9146000 - 0.004817*t - 0.000014*t*t)*Math.sin(mr)
            + (0.019993 - 0.000101*t)*Math.sin(2*mr)
            + 0.000290*Math.sin(3*mr);
    var theta = l0 + c;
    var nu = m + c;
    var r = (1.000001018*(1-e*e))/(1+e*Math.cos(riseset.d2r(nu)));
    var omega = 125.04 - 1934.136*t;
    var lambda = theta - 0.00569 - 0.00478*Math.sin(riseset.d2r(omega));
    var eps = riseset.obliquityOfEcliptic(ajd) + 0.00256*Math.cos(riseset.d2r(omega));
    var alpha = Math.atan2(Math.cos(riseset.d2r(eps))*Math.sin(riseset.d2r(lambda)),Math.cos(riseset.d2r(lambda)));
    var delta = Math.asin(Math.sin(riseset.d2r(eps))*Math.sin(riseset.d2r(lambda)));
    return [riseset.r2d(alpha),riseset.r2d(delta)];
};
